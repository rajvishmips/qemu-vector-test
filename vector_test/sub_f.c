...
static int
rvv_subf_v( float a[], float b[], float c[],int n)
{
    size_t vl=0;
    size_t i=0;
    while(n>0) {
...
static int
rvv_subf_s(float a[], float b[], float c[],int n)
{
    int i;
    for(i=0;i<n;i++) {
        c[i]=a[i]-b[i];
...
test_subf(void) 
{
    ...
    rvv_subf_v(a,b,c_v,MAX_ELEMENTS);
    rvv_subf_s(a,b,c_s,MAX_ELEMENTS);
    if (!check_array_f(c_v,c_s,MAX_ELEMENTS)) {
        printf("Float Subtracvtion  Failed. \n");
        print_result_f(c_v,c_s,MAX_ELEMENTS);
        return false;
    }
    printf("Float Subtraction succesful \n");
    return true;
}
